...
b_ss_get <- function(self, private, service, username, keyring) {
  ...
  res <- .Call(c_keyring_secret_service_get, keyring, service, username)
  if (any(res == 0)) {
...
b_ss_set_with_value <- function(self, private, service, username, password,
                                keyring) {
  keyring <- keyring %||% private$keyring
  .Call(c_keyring_secret_service_set, keyring, service, username,
        charToRaw(password))
  invisible(self)
}
...
b_ss_delete <- function(self, private, service, username, keyring) {
  keyring <- keyring %||% private$keyring
  .Call(c_keyring_secret_service_delete, keyring, service, username)
  invisible(self)
}

b_ss_list <- function(self, private, service, keyring) {
  keyring <- keyring %||% private$keyring
  res <- .Call(c_keyring_secret_service_list, keyring, service)
  data.frame(
    service = res[[1]],
    username = res[[2]],
...
b_ss_keyring_list <- function(self, private) {
  res <- .Call(c_keyring_secret_service_list_keyring)
  data.frame(
    keyring = res[[1]],
    num_secrets = res[[2]],
    locked = res[[3]],
...
b_ss_keyring_delete <- function(self, private, keyring) {
  keyring <- keyring %||% private$keyring
  .Call(c_keyring_secret_service_delete_keyring, keyring)
  invisible()
}

b_ss_keyring_lock <- function(self, private, keyring) {
  keyring <- keyring %||% private$keyring
  .Call(c_keyring_secret_service_lock_keyring, keyring)
  invisible()
}
...
b_ss_keyring_unlock <- function(self, private, keyring, password) {
  ...
  .Call(c_keyring_secret_service_unlock_keyring, keyring, password)
  invisible()
}
...
b_ss_is_available <- function(self, private, report_error) {
  .Call(c_keyring_secret_service_is_available, report_error)
}
